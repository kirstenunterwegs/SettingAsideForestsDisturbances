...
#
# code for sensitivity analysis of the disturbance severity cutoff point for high 
# and low disturbance severity
#
#################################################################################
...
#high.severity.map <- terra::rast("data/processed/disturbances/germany/high.severity.natural_0.7.tif") # natural disturbances only
high.severity.map <- terra::rast("data/processed/disturbances/germany/high.severity.natural_0.9.tif") # natural disturbances only
...
# rasterize reserves and NPs (set aside/unmanaged forests)
...
# stack disturbance information and crop to control and reserach area
...
#writeRaster(Research_disturbance_stack, "data/processed/disturbances/dist_stack_managed_unmanaged/research_stack_nat.agent_0.7_severity.tif")
#writeRaster(Control_disturbance_stack, "data/processed/disturbances/dist_stack_managed_unmanaged/control_stack_nat.agent_0.7_severity.tif")

writeRaster(Research_disturbance_stack, "data/processed/disturbances/dist_stack_managed_unmanaged/research_stack_nat.agent_0.9_severity.tif")
writeRaster(Control_disturbance_stack, "data/processed/disturbances/dist_stack_managed_unmanaged/control_stack_nat.agent_0.9_severity.tif")
...
#Research_disturbance_stack <- rast("data/processed/disturbances/dist_stack_managed_unmanaged/research_stack_nat.agent_0.7_severity.tif")
#Control_disturbance_stack <- rast("data/processed/disturbances/dist_stack_managed_unmanaged/control_stack_nat.agent_0.7_severity.tif")

Research_disturbance_stack <- rast("data/processed/disturbances/dist_stack_managed_unmanaged/research_stack_nat.agent_0.9_severity.tif")
Control_disturbance_stack <- rast("data/processed/disturbances/dist_stack_managed_unmanaged/control_stack_nat.agent_0.9_severity.tif")
...
# --- assign the window size  and fcover of the research site to attain window for extracting control site disturbance info

# assign fcover of reserach sublandscape to control sublandscape (to compare extracted fcover in control site to reserach site)
export_df <- export_df %>%
  group_by(subclass) %>%
  #mutate(fcover = fcover[Research == 1])
  mutate(fcover = fcover[which(Research == 1)[1]]) # to deal with pseudo replication in 1:Many sampling design of unmanaged sublandscapes
...
#saveRDS(dist.climate.df, paste0("data/processed/dist.extraction/",folder,"/match.df_climate_dist_0.7_severity.rds"))
saveRDS(dist.climate.df, paste0("data/processed/dist.extraction/",folder,"/match.df_climate_dist_0.9_severity.rds"))
